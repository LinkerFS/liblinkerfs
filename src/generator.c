...
static void write_header(const WARP_CONFIG *warp_config, const int64_t size, unsigned char *buf) {
    LINKERFS_HEADER header = {.part_info_begin_offset = header_length,
                              .warp_size = size,
...
static void write_part_info(const WARP_TARGET *target, LINKERFS_PART *part, const PATH_INFO_HELPER *path_helper_array,
                            const int32_t path_offset) {
...
static void write_path_info(const WARP_TARGET *target, unsigned char *path_info_begin,
                            const PATH_INFO_HELPER *path_helper) {
...
WARP_FILE generate_warp_file(const WARP_CONFIG *warp_config) {
    ...
    const int64_t total_size =
            write_part_info_and_path_info(warp_config, warp_file.data + header_length, path_helper, part_info_size);
